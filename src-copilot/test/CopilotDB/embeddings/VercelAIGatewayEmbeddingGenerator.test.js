...
describe('Vercel AI Gateway Embedding', () => {
    beforeEach(() => {
        allure.epic('src-copilot');
    });

    test.skipIf(!process.env.VERCELAIGATEWAY_API_KEY, 'works with single string', async () => {
        const embeddingGenerator = new VercelAIGatewayEmbeddingGenerator();
        const app = mockApp();
    ...
    test.skipIf(!process.env.VERCELAIGATEWAY_API_KEY, 'works with array', async () => {
        const embeddingGenerator = new VercelAIGatewayEmbeddingGenerator();
        const app = mockApp();
        app.settings[EMBEDDING_API_KEY_SETTING] = process.env.VERCELAIGATEWAY_API_KEY;
